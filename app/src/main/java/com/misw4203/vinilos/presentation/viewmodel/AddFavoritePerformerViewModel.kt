...
class AddFavoritePerformerViewModel @Inject constructor(
    ...
    fun onAddMusician(musicianId: Int) {
        ...
        viewModelScope.launch {
            when (runCatchingDomain { addFavoriteMusician(collectorId, musicianId) }) {
                is DomainResult.Ok -> {
    ...
    fun onAddBand(bandId: Int) {
        ...
        viewModelScope.launch {
            when (runCatchingDomain { addFavoriteBand(collectorId, bandId) }) {
                is DomainResult.Ok -> {
    ...
    private fun loadInitial() {
        ...
        viewModelScope.launch {
            when (runCatchingDomain {
                ...
                DomainResult.Network -> _uiState.value = AddFavoritePerformerUiState.Error(
                    isNetworkError = true, type = null, performerId = null,
                )
                DomainResult.NotFound -> _uiState.value = AddFavoritePerformerUiState.Error(
                    isNetworkError = false, type = null, performerId = null,
                )
                DomainResult.Server -> _uiState.value = AddFavoritePerformerUiState.Error(
                    isNetworkError = false, type = null, performerId = null,
                )
            }
        }
